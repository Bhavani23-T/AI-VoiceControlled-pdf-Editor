...
async function parsePdfWithLlamaParse(fileBuffer: ArrayBuffer, filename: string, apiKey: string): Promise<string> {
  ...
  if (!uploadRes.ok) {
    throw new Error(`Failed to upload to LlamaParse: ${await uploadRes.text()}`);
  }
  ...
  let status = "PENDING";
  let iterations = 0;
  while ((status === "PENDING" || status === "IN_PROGRESS") && iterations < 60) {
  ...
  if (!resultRes.ok) {
    throw new Error(`Failed to fetch parsed markdown: ${await resultRes.text()}`);
  }
...
export async function POST(req: Request) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({ detail: "LLAMAPARSE_API_KEY is not configured on the server." }, { status: 500 });
    }

    const arrayBuffer = await file.arrayBuffer();

    if (arrayBuffer.byteLength > 25 * 1024 * 1024) {
      return NextResponse.json({ detail: "File too large. Maximum size is 25MB." }, { status: 413 });
    }

    try {
      // 1. Extract markdown using LlamaParse REST API
      const markdown = await parsePdfWithLlamaParse(arrayBuffer, file.name, apiKey);
